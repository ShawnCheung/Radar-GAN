...
if is_training:
    ...
    def prepare_data(train_path):
        ...
        for dirname in train_path:
            train_t_gt = dirname + "/train-c/"
            train_b = dirname + "/train-k/"
            for root, _, fnames in sorted(os.walk(train_t_gt)):
                for fname in fnames:
                    if is_image_file(fname):
                        path_input = os.path.join(train_b, fname)
                        path_output1 = os.path.join(train_b, fname)
                        path_output2 = os.path.join(train_t_gt, fname)
                        input_names.append(path_input)   #names of train-k
                        image1.append(path_output1)    #names of train-k
                        image2.append(path_output2)    #names of train-c
    ...
    for epoch in range(1,maxepoch):
        ...
        for id in np.random.permutation(num_train):
            ...
            if input_images[id] is None:
                ...
                inputimg = cv2.imread(input_real_names[_id])
                ...
                output_image_t=cv2.resize(np.float32(cv2.imread(output_real_names2[_id])),(neww,newh),cv2.INTER_CUBIC)/255.0
                ...
                fetch_list=[g_opt,transmission_layer,
                    d_loss,g_loss,loss]
                # update G
                _,pred_image_t,current_d,current_g,current=\
                    sess.run(fetch_list,feed_dict={input:input_images[id],target:output_images_t[id],issyn:is_syn})
        ...
        if epoch % ARGS.save_model_freq == 0:
            ...
            fileid = os.path.splitext(os.path.basename(input_real_names[id]))[0]
            ...
            print("shape of outputs: ",pred_image_t.shape)
